...
class MailRenderMixin(models.AbstractModel):
    ...
    def update_brand_color(self, value):
        if len(value) < 20:
            return value

        old_theme_color_brand = self.env[
                "res.config.settings"
            ].old_mail_theme_primary
        theme_color_brand = self.env[
                "res.config.settings"
            ].mail_theme_primary

        return re.sub(old_theme_color_brand, theme_color_brand, 
